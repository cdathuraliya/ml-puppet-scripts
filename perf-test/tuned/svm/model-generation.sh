...
# create the dataset
path=$(pwd)
curl -s -X POST -b cookies  https://$SEVER_IP:9443/api/datasets -H "Authorization: Basic YWRtaW46YWRtaW4=" -H "Content-Type: multipart/form-data" -F datasetName='pimaIndiansDiabetes-svm-dataset' -F version='2.0.0' -F description='Pima Indians Diabetes Dataset' -F sourceType='file' -F destination='file' -F dataFormat='CSV' -F containsHeader='true' -F file=@'/'$path'/IndiansDiabetes.csv' -k
sleep 5
...
# get the project
project=$(curl -s -H "Content-Type: application/json" -H "Authorization: Basic YWRtaW46YWRtaW4=" https://$SEVER_IP:9443/api/projects/wso2-ml-svm-tuned-sample-project -k)
sleep 2
...
# get analysis ID
analysis=$(curl -s -H "Content-Type: application/json" -H "Authorization: Basic YWRtaW46YWRtaW4=" https://$SEVER_IP:9443/api/projects/${projectId}/analyses/wso2-ml-svm-tuned-sample-analysis -k)
sleep 2
...
curl -s -X POST -d @'hyper-parameters' -H "Content-Type: application/json" -H "Authorization: Basic YWRtaW46YWRtaW4=" https://$SEVER_IP:9443/api/analyses/${analysisId}/hyperParams?algorithmName=SVM -k
sleep 2
