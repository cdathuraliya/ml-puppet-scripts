...
# create the dataset
path=$(pwd)
curl -s -X POST -b cookies  https://$SEVER_IP:9443/api/datasets -H "Authorization: Basic YWRtaW46YWRtaW4=" -H "Content-Type: multipart/form-data" -F datasetName='seeds' -F version='1.0.0' -F description='Seeds Dataset' -F sourceType='file' -F destination='file' -F dataFormat='CSV' -F containsHeader='true' -F file=@'/'$path'/seeds.csv' -k
sleep 5
...
# get the project
project=$(curl -s -H "Content-Type: application/json" -H "Authorization: Basic YWRtaW46YWRtaW4=" https://$SEVER_IP:9443/api/projects/wso2-ml-kmeans-sample-project -k)
sleep 2
...
# get analysis ID
analysis=$(curl -s -H "Content-Type: application/json" -H "Authorization: Basic YWRtaW46YWRtaW4=" https://$SEVER_IP:9443/api/projects/${projectId}/analyses/wso2-ml-kmeans-sample-analysis -k)
sleep 2
